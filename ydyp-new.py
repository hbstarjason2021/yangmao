###### from https://github.com/Xx1aoy1/scripts/blob/ef5124efaa154925c71907712fa7811e8a7c517b/zgydyp5.0.2.py


# @file  中国移动云盘
# @desp  签到 基础任务 果园 云朵大作战
# @version v5.0.2
# @author github@villagebully8 https://github.com/villagebully8
# 使用说明:
#   - [抓包 Cookie：任意Authorization]
#   - [注意事项: 简易方法，开抓包进App，搜refresh，找到authTokenRefresh.do ，请求头中的Authorization]
# 环境变量设置:
...
# https://raw.githubusercontent.com/villagebully8/yybs/main/%E4%BA%91%E7%9B%98%E4%BA%91%E6%9C%B55.0.2.py
import os
import random
import re
import time
...
ua = "Mozilla/5.0 (iPhone; CPU iPhone OS 17_4_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) MCloudApp/11.0.1 iPhone AppLanguage/zh-CN"
...
class YP:
    ...
    def send_request(self, url, headers=None, cookies=None, data=None, method='GET', debug=None, retries=5):
    ...
    def sso(self):
        ...
        sso_data = self.send_request(
            sso_url, headers=sso_headers, data=sso_payload, method="POST"
        ).json()
    ...
    def jwt(self):
        ...
        if token is not None:
            ...
            jwt_data = self.send_request(
                jwt_url, headers=self.jwtHeaders, method="POST"
            ).json()
            if jwt_data["code"] != 0:
                print(jwt_data["msg"])
                return False
    ...
    def signin_status(self):
        ...
        try:
            check_data = self.send_request(
                check_url, headers=self.jwtHeaders, cookies=self.cookies
            ).json()
            if check_data["msg"] == "success":
                ...
                if today_sign_in:
                    ...
                    signin_data = self.send_request(
                        signin_url, headers=self.jwtHeaders, cookies=self.cookies
                    ).json()
    ...
    def click(self):
        ...
        try:
            for _ in range(self.click_num):
                return_data = self.send_request(
                    url, headers=self.jwtHeaders, cookies=self.cookies
                ).json()
                time.sleep(0.2)
                ...
                if app_type == "cloud_app":
                    if task_type == "month":
                        ...
                        for month in tasks:
                            ...
                            self.do_task(
                                task_id, task_type="month", app_type="cloud_app"
                            )
                            time.sleep(2)
                    elif task_type == "day":
                        print("\n📆 云盘每日任务")
                    ...
                    if task_type == "month":
                        ...
                        for month in tasks:
                            ...
                            print(f"-去完成: {task_name}")
                            self.do_task(
                                task_id, task_type="month", app_type="email_app"
                            )
                            time.sleep(2)
        except Exception as e:
            print(f"错误信息:{e}")
    ...
    def surplus_num(self):
        ...
        try:
            draw_info_data = self.send_request(
                draw_info_url, headers=self.jwtHeaders
            ).json()
                ...
                if remain_num > 50 - self.draw:
                    for _ in range(self.draw):
                        ...
                        draw_data = self.send_request(
                            url=draw_url, headers=self.jwtHeaders
                        ).json()
    ...
    def fruitLogin(self):
        ...
        if token is not None:
            ...
            doLoginData = self.send_request(
                do_login_url, headers=self.treeHeaders
            ).json()
    ...
    def fruitTask(self):
        # 签到任务
        check_sign_data = self.send_request(
            f"{self.fruit_url}task/checkinInfo.do", headers=self.treeHeaders
        ).json()
        if check_sign_data.get("success"):
            ...
            if today_checkin == 1:
                ...
                checkin_data = self.send_request(
                    f"{self.fruit_url}task/checkin.do", headers=self.treeHeaders
                ).json()
                if checkin_data.get("result", {}).get("code", "") == 1:
                    print("-果园签到成功")
                self.sleep()
                water_data = self.send_request(
                    f"{self.fruit_url}user/clickCartoon.do?cartoonType=widget",
                    headers=self.treeHeaders,
                ).json()
                color_data = self.send_request(
                    f"{self.fruit_url}user/clickCartoon.do?cartoonType=color",
                    headers=self.treeHeaders,
                ).json()
        ...
        # 获取任务列表
        task_list_data = self.send_request(
            f"{self.fruit_url}task/taskList.do?clientType=PE", headers=self.treeHeaders
        ).json()
        task_state_data = self.send_request(
            f"{self.fruit_url}task/taskState.do", headers=self.treeHeaders
        ).json()
        ...
        for task in task_list:
            ...
            task_state = next(
                (
                    state.get("taskState", 0)
                    for state in task_state_result
    ...
    def do_fruit_task(self, task_name, task_id, water_num):
        ...
        if do_task_data.get("success"):
            ...
            get_water_data = self.send_request(
                get_water_url, headers=self.treeHeaders
            ).json()
    ...
    def tree_info(self):
        ...
        if not treeinfo_data.get("success"):
            ...
            if tree_level in (2, 4, 6, 8):
                ...
                openbox_data = self.send_request(
                    openbox_url, headers=self.treeHeaders
                ).json()
            ...
            if watering_amount > 0:
                for _ in range(watering_amount):
                    watering_data = self.send_request(
                        watering_url, headers=self.treeHeaders
                    ).json()
    ...
    def cloud_game(self):
        game_info_url = (
            "https://caiyun.feixin.10086.cn/market/signin/hecheng1T/info?op=info"
        )
        bigin_url = "https://caiyun.feixin.10086.cn/market/signin/hecheng1T/beinvite"
        end_url = (
            "https://caiyun.feixin.10086.cn/market/signin/hecheng1T/finish?flag=true"
        )

        game_info_data = self.send_request(
            game_info_url, headers=self.jwtHeaders, cookies=self.cookies
        ).json()
        if game_info_data and game_info_data.get("code", -1) == 0:
            currnum = game_info_data.get("result", {}).get("info", {}).get("curr", 0)
            count = (
                game_info_data.get("result", {})
                .get("history", {})
                .get("0", {})
                .get("count", "")
            )
            ...
            for _ in range(currnum):
                self.send_request(
                    bigin_url, headers=self.jwtHeaders, cookies=self.cookies
                ).json()
                print("-开始游戏,等待10-15秒完成游戏")
                time.sleep(random.randint(10, 15))
                end_data = self.send_request(
                    end_url, headers=self.jwtHeaders, cookies=self.cookies
                ).json()
    ...
    def receive(self):
        ...
        try:
            receive_data = self.send_request(
                receive_url, headers=self.jwtHeaders, cookies=self.cookies
            ).json()
            self.sleep()
            prize_data = self.send_request(
                prize_url, headers=self.jwtHeaders, cookies=self.cookies
            ).json()
    ...
    def backup_cloud(self):
        ...
        expend_data = self.send_request(
            expend_url, headers=self.jwtHeaders, cookies=self.cookies
        ).json()

        curMonthBackup = expend_data.get("result", {}).get(
            "curMonthBackup", ""
        )  # 本月备份
        preMonthBackup = expend_data.get("result", {}).get(
            "preMonthBackup", ""
        )  # 上月备份
    ...
    def open_send(self):
        ...
        pushOn = send_data.get("result", {}).get(
            "pushOn", ""
        )  # 0未开启，1开启，2未领取，3已领取
        ...
        if pushOn == 1:
            ...
            if firstTaskStatus == 3:
                ...
                # 领取任务1
                print("- 领取任务1奖励")
                reward1_data = self.send_request(
                    reward_url, headers=self.jwtHeaders, data={"type": 1}, method="POST"
                ).json()
            ...
            if secondTaskStatus == 2:
                # 领取任务2
                print("- 领取任务2奖励")
                reward2_data = self.send_request(
                    reward_url, headers=self.jwtHeaders, data={"type": 2}, method="POST"
                ).json()
